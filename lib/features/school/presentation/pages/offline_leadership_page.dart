...
class OfflineLeadershipPage extends StatefulWidget {
  const OfflineLeadershipPage({super.key});

  @override
  State<OfflineLeadershipPage> createState() => _OfflineLeadershipPageState();
}

class _OfflineLeadershipPageState extends State<OfflineLeadershipPage> {
  ...
  Future<void> _loadPending() async {
    setState(() {
      _pendingLeadership = LocalStorageService.getPendingLeadership();
    });
  }
  ...
  Future<void> _syncAll() async {
    ...
    int successCount = 0;
    final copy = List<Map<String, dynamic>>.from(_pendingLeadership);
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(successCount > 0 ? '$successCount leadership assessment(s) synced!' : 'No assessments synced.'),
  ...
  Future<void> _deletePending(int index) async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: const Text('Delete Pending Leadership Assessment'),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(
        title: 'Offline Leadership Assessments',
        ...
        builder: (context, snapshot) {
          ...
          return Column(
            children: [
              ...
              Expanded(
                child: _pendingLeadership.isEmpty
                    ? Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      const Text('No Pending Leadership Assessments', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                  ...
                  child: ListView.builder(
                    ...
                    itemCount: _pendingLeadership.length,
      ...
      floatingActionButton: _pendingLeadership.isNotEmpty
          ? StreamBuilder<bool>(
        stream: LocalStorageService.onlineStatusStream,
        initialData: LocalStorageService.currentOnlineStatus,
        builder: (context, snapshot) {
          ...
          return FloatingActionButton.extended(
            ...
            label: Text(_isSyncing ? 'Syncing...' : 'Sync All (${_pendingLeadership.length})', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w600)),
          );
        },
      )
          : null,
