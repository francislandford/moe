...
class _AddSchoolPageState extends State<AddSchoolPage> {
  ...
  String? _firstAssessment = 'Yes'; // First assessment field

  bool _isTvet = false;
  bool _isAccelerated = false;
  bool _isAlternative = false;
  bool _isInclusive = false; // NEW: Inclusive and Special School field
  ...
  // Generate school code in format MOE-XXX-YYY
  ...
  Future<void> _submitForm() async {
    ...
    // Log the current schoolClosed value before submission
    debugPrint('Current schoolClosed value before submission: $_schoolClosed');

    // Check if school is closed OR compliance is No
    if (_isSchoolClosed || (_showCompliance && !_hasConsent)) {
      // Save minimal school data
      final minimalSchoolData = <String, dynamic>{
        ...
        // Add user's current date and time
        'date': now.toIso8601String(),
      };

      // Only include compliance if school is NOT closed - otherwise don't send it at all
      if (_showCompliance) {
        minimalSchoolData['compliance'] = _consent;
      }
      // If school is closed, we don't send the compliance field at all

      debugPrint('Submitting minimal school data (school closed or no consent): $minimalSchoolData');
      ...
      try {
        ...
        if (result['success'] == true) {
          ...
          if (mounted) {
            ...
            context.push('/assessment-complete', extra: {
              'isOffline': result['offline'] == true,
              'schoolName': _schoolNameController.text.trim(),
            });
          }
        } else {
          _showErrorDialog(result['message'] ?? 'Failed to create school: Server error');
        }
      } catch (e) {
    ...
    // Original full submission logic for schools that are open AND have consent
    ...
    if (_showCompliance && _hasConsent) {
      schoolData.addAll({
        ...
        'inclusive': _isInclusive ? '1' : '0', // NEW: Add inclusive field
                            ...
                            DropdownButtonFormField<String>(
                              ...
                              items: provider.districts.isEmpty
                                ...
                                return DropdownMenuItem<String>(
                                  value: m['d_name']?.toString() ?? '',
                                  child: Text(m['d_name']?.toString() ?? 'Unknown'),
                                );
                              }).toList(),
                          ...
                          children: [
                            ...
                            DropdownButtonFormField<String>(
                              ...
                              items: provider.levels.isEmpty
                                ...
                                return DropdownMenuItem<String>(
                                  ...
                                  child: Text(level['name']?.toString() ?? 'Unknown'),
                                );
                              }).toList(),
                            ...
                            _dropdown(
                              ...
                              items: provider.types.isEmpty
                                  ? ['Loading...']
                                  : provider.types.map((m) => m['name']?.toString() ?? 'Unknown').toList(),
                              onChanged: (v) {
                                if (v != 'Loading...' && v != null) {
                            ...
                            _dropdown(
                              ...
                              items: provider.ownerships.isEmpty
                                  ? ['Loading...']
                                  : provider.ownerships.map((m) => m['name']?.toString() ?? 'Unknown').toList(),
                              onChanged: (v) {
                                if (v != 'Loading...' && v != null) {
                            ...
                            // Is school Closed dropdown
                            DropdownButtonFormField<String>(
                              key: ValueKey('school_closed_${_schoolClosed}'), // Force rebuild on value change
                              value: _schoolClosed,
                              decoration: const InputDecoration(
                              ...
                              onChanged: (v) {
                                setState(() {
                                  _schoolClosed = v;
                                  // When school is closed, reset consent to No
                                  if (_isSchoolClosed) {
                                    _consent = 'No';
                                  }
                                });
                            ...
                            if (_showCompliance) ...[
                              DropdownButtonFormField<String>(
                                key: ValueKey('consent_${_consent}'), // Force rebuild on value change
                                value: _consent,
                                decoration: const InputDecoration(
                                  labelText: 'Compliant *', // Changed spelling
                                  border: OutlineInputBorder(),
                                ),
                                items: const ['Yes', 'No']
                                    .map((item) => DropdownMenuItem<String>(
                        ...
                        if (_showCompliance && _hasConsent) ...[
                          ...
                          _buildSectionCard(
                            ...
                            children: [
                              ...
                              // NEW: Inclusive and Special School
                              SwitchListTile(
                                title: const Text('Inclusive and Special School'),
                                value: _isInclusive,
                          ...
                          _buildSectionCard(
                            ...
                            children: [
                              DropdownButtonFormField<String>(
                                ...
                                decoration: const InputDecoration(
                                  labelText: 'Is this your first assessment at this school?',
                                  border: OutlineInputBorder(),
                                ),
                                items: const ['Yes', 'No']
                                    .map((item) => DropdownMenuItem<String>(
  ...
  Widget _yearDropdown() {
    ...
    return DropdownButtonFormField<String>(
      ...
      validator: _hasConsent ? (v) => v == null ? 'Please select a year' : null : null,
    );
  }
}