...
class _OfflineParentParticipationPageState extends State<OfflineParentParticipationPage> {
  ...
  List<Map<String, dynamic>> _pendingParent = [];
  ...
  Future<void> _loadQuestionsFromCache() async {
    final cachedList = LocalStorageService.getFromCache('parent_questions');
  ...
  Future<void> _syncAll() async {
    ...
    int successCount = 0;
    final copy = List<Map<String, dynamic>>.from(_pendingParent);
    ...
    for (var item in copy) {
      try {
        ...
        debugPrint('Parent sync failed: $e');
      }
    }
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(successCount > 0 ? '$successCount parent participation(s) synced!' : 'No entries synced.'),
  ...
  Future<bool> _syncSingle(Map<String, dynamic> payload) async {
    ...
    try {
      final res = await http.post(
        Uri.parse('${AppUrl.url}/parent-participation'), // Adjust endpoint as needed
        headers: headers,
  ...
  Future<void> _removeFromPending(Map<String, dynamic> toRemove) async {
    final current = await ParentLocalStorageService.getPending();
    final updated = current.where((p) => p['queuedAt'] != toRemove['queuedAt']).toList();

    // Save updated list back to storage
    final box = await Hive.openBox<List>('parent_pending_box');
    await box.put('pending', updated);

    setState(() => _pendingParent = updated);
  }
  ...
  Future<void> _updatePending(Map<String, dynamic> updatedItem) async {
    ...
    if (index != -1) {
      ...
      setState(() {
        _pendingParent = current;
      });

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Parent participation updated successfully'),
  ...
  Future<void> _deletePending(int index) async {
    final item = _pendingParent[index];
  ...
  // Edit dialog for offline parent data
    ...
    showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setState) {
          return AlertDialog(
            title: Text('Edit Parent Participation: ${editableItem['school'] ?? 'Unnamed'}'),
            content: Container(
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    // Parent Participation Questions
                    const Text('Parent Participation Questions', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                    const SizedBox(height: 8),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(
        title: 'Offline Parent Participation',
        backgroundColor: AppColors.primary,
        leading: IconButton(
              ...
              Expanded(
                child: _pendingParent.isEmpty
                  ...
                  child: ListView.builder(
                    ...
                    itemCount: _pendingParent.length,
                        ...
                        child: InkWell(
                          ...
                          child: ListTile(
                            ...
                            leading: CircleAvatar(
                              ...
                              child: Icon(Icons.people_rounded, color: AppColors.primary, size: 32),
                            ),
      ...
      floatingActionButton: _pendingParent.isNotEmpty
          ? StreamBuilder<bool>(
        stream: LocalStorageService.onlineStatusStream,
        initialData: LocalStorageService.currentOnlineStatus,
        builder: (context, snapshot) {
          ...
          return FloatingActionButton.extended(
            ...
            label: Text(_isSyncing ? 'Syncing...' : 'Sync All (${_pendingParent.length})', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w600)),
          );
        },
      )
          : null,
